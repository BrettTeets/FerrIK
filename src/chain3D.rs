...
use crate::{bone3D, Joint3D};

#[derive(Clone)]
pub struct Chain3D{
    id: usize,
    m_chain: Vec<Bone3D>,
    mSolveDistanceThreshold: f32, //this was set to 1.0 as the default.
...
impl Chain for Chain3D{
    fn addBone(&mut self, bone: bone::Bone3D)
	{
    ...
    fn add_consecutive_bone_by_vector(Vec3f directionUV, float length)
	{
    ...
    fn add_consecutive_bone_with_bone(FabrikBone3D bone)
	{
		// Validate the direction unit vector - throws an IllegalArgumentException if it has a magnitude of zero
...
pub enum BaseboneConstraintType3D {
	...
	public void addConsecutiveRotorConstrainedBone(Vec3f boneDirectionUV, float boneLength, float constraintAngleDegs, Colour4f colour)
	{
		...
		FabrikBone3D bone = new FabrikBone3D(mChain.get(mChain.size()-1).getEndLocation(), boneDirectionUV.normalise(), boneLength, colour);
	 ...
	 */
	@Override
	public Vec3f getEffectorLocation() { return mChain.get(mChain.size()-1).getEndLocation(); }
